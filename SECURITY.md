...
This is the security policy for **Drug Discovery Agent Skills**, maintained by [K-Dense](https://www.k-dense.ai).
...
If you pin skills to a tag or commit (see [Version pinning](README.md#-version-pinning) in the README), you are responsible for moving that pin forward to receive fixes.
...
- **Vulnerabilities in the third-party libraries and services a skill documents.** A flaw in RDKit, DeepChem, or a public API belongs to that project — please report it upstream. A skill that *instructs users to use a library unsafely* is in scope here.
- **Vulnerabilities in agent hosts.** Issues in Claude Code, Cursor, Codex, and similar belong to those vendors.
- **The inherent capability of skills.** Skills are instructions for an agent that can execute code; a skill performing the work it documents is not a vulnerability. See the [Security disclaimer](README.md#-security-disclaimer) in the README.
...
- **Workflow:** [`.github/workflows/security-scan.yml`](.github/workflows/security-scan.yml) — the scheduled scan
- **Report:** each successful scheduled run commits `docs/security-report.md` and its machine-readable companion `docs/security-report.json` to the default branch, and attaches both as a `security-report` workflow artifact. If `docs/` is not present in your checkout, no scheduled scan has completed yet.
- **Per-PR scan:** [`.github/workflows/pr-skill-scan.yml`](.github/workflows/pr-skill-scan.yml) posts its results as a sticky comment on the pull request rather than committing a file
...
If you find a skill published *outside* this repository that impersonates Drug Discovery Agent Skills or K-Dense, please tell us through the private reporting channel above so we can respond and warn users.
